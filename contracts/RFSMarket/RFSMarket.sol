...
import "./RFS/AbstractRFS.sol";
import "../utils/strings.sol";
import "../utils/StringUtils.sol";

contract RFSMarket is StandardMarket {
	using strings for *;
	...
	// Constructor
	function RFSMarket(Kiosk _kiosk, AbstractRFS _rfsm) StandardMarket(_kiosk) {
		rfsm = _rfsm;
	}
...

	{
		...
		rfsm.setOwner(domains[DIN].node, buyer);
	...
	function availableForSale(uint256 DIN, uint256 quantity, address buyer) constant returns (bool) {
		...
		if (rfsm.owner(node) != address(this)) {
			return false;
		}
