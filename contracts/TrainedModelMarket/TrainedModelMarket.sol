...
import "./TrainedModel/AbstractTrainedModel.sol";
import "../utils/strings.sol";
import "../utils/StringUtils.sol";

contract TrainedModelMarket is StandardMarket {
	using strings for *;
	using StringUtils for *;

	string public name = "Notebook Market";

	// TrainedModel Registry
	AbstractTrainedModel public tmm;
	...
	// DIN => TrainedModel Node
	mapping(uint256 => Domain) public domains;

	// Buyer => TrainedModel Node
	...
	// Constructor
	function TrainedModelMarket(Kiosk _kiosk, AbstractTrainedModel _tmm) StandardMarket(_kiosk) {
		tmm = _tmm;
	}
...

	{
		...
		tmm.setOwner(domains[DIN].node, buyer);
	...
	function availableForSale(uint256 DIN, uint256 quantity, address buyer) constant returns (bool) {
		...
		if (tmm.owner(node) != address(this)) {
			return false;
		}
